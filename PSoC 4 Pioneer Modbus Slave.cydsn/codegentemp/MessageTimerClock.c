...
void MessageTimerClock_SetFractionalDividerRegister(uint16 clkDivider, uint8 clkFractional)
{
    ...
    uint32 maskVal = MessageTimerClock_DIV_REG & 
                    (uint32)(~(MessageTimerClock__DIVIDER_MASK | MessageTimerClock__FRAC_MASK)); 
	/* combine mask and new divider val into 32-bit value */
    uint32 regVal = clkDivider | (((uint32)clkFractional << 16) & MessageTimerClock__FRAC_MASK) | maskVal;
#else
    /* get all but integer divider bits */
    uint32 maskVal = MessageTimerClock_DIV_REG & (uint32)(~(uint32)MessageTimerClock__DIVIDER_MASK);
    /* combine mask and new divider val into 32-bit value */
...
{
    return (uint16)MessageTimerClock_DIV_REG;
}
...
uint8 MessageTimerClock_GetFractionalDividerRegister(void)
{
    ...
    uint32 maskVal = MessageTimerClock_DIV_REG & MessageTimerClock__FRAC_MASK;
    return (maskVal >> 16u);
#else
    return 0u;
#endif /* MessageTimerClock__FRAC_MASK */
